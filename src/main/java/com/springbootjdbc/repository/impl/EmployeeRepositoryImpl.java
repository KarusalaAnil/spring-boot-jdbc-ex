...
public class EmployeeRepositoryImpl implements EmployeeRepository {
    ...
    public Employee findById(int id) {
        return jdbcTemplate.queryForObject("SELECT * FROM EMPLOYEE_DATA WHERE id =? ",
                new BeanPropertyRowMapper<>(Employee.class),
                id);
    }
    ...
    public String getNameById(int id) {
        return jdbcTemplate.queryForObject("SELECT name FROM EMPLOYEE_DATA WHERE id =?",
                String.class, id);
    }

    @Override
    public List<Employee> findByNameAndDept(String name, String dept) {
        return jdbcTemplate.query("SELECT * FROM EMPLOYEE_DATA WHERE name = ? and dept =? ",
